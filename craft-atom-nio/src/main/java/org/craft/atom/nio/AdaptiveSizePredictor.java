...
/**
 * The {@link AdaptiveSizePredictor} that automatically increases and decreases the predicted buffer size on feed back.
 * <p>
 ...
 */
public class AdaptiveSizePredictor implements SizePredictor {
     ...
     */
    public AdaptiveSizePredictor(int minimum, int initial, int maximum) {
        if (minimum <= 0) {
	...
	public String toString() {
		return String
				.format("AdaptiveSizePredictor [minIndex=%s, maxIndex=%s, index=%s, nextSize=%s, decreaseNow=%s]",
						minIndex, maxIndex, index, nextSize, decreaseNow);
	}
