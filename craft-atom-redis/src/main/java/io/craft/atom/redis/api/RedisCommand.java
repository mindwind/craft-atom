...
public interface RedisCommand {
	 ...
	 * 
	 * <p>
	 * <b>Details on strings comparison</b>
     ...
     * However the user can apply a transformation to the encoded string so that the first part of the element inserted in the sorted set 
     * will compare as the user requires for the specific application. For example if I want to add strings that will be compared in a case-insensitive way, 
     ...
     * we are forcing a given comparison, however after the range is queries using ZRANGEBYLEX the application can display 
     * to the user the second part of the string, after the colon.
     * The binary nature of the comparison allows to use sorted sets as a general purpose index, 
     * for example the first part of the element can be a 64 bit big endian number: since big endian numbers have the most significant bytes in the initial positions, 
     ...
     * 
     * <b>HyperLogLog representation</b><br>
	 ...
	 * 
	 * <p>
	 * The CLIENT KILL command closes a given client connection. Up to Redis 2.8.11 it was possible to close a connection only by client address, 
	 ...
	 * <ul>
	 * <li>CLIENT KILL ADDR ip:port. This is exactly the same as the old three-arguments behavior.</li>
	 * <li>CLIENT KILL ID client-id. Allows to kill a client by its unique ID field, which was introduced in the CLIENT LIST command starting from Redis 2.8.12.</li>
     * <li>CLIENT KILL TYPE type, where type is one of normal, slave, pubsub. This closes the connections of all the clients in the specified class. Note that clients blocked into the MONITOR command are considered to belong to the normal class.</li>
     * <li>CLIENT KILL SKIPME yes/no. By default this option is set to yes, that is, the client calling the command will not get killed, however setting this option to no will have the effect of also killing the client calling the command.</li>
	 * </ul>
     ...
     * 
     * <b>CLIENT KILL and Redis Sentinel</b>
